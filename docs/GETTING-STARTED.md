# Reactor V installation and development guide
...
| --- | --- | --- |
| `ReactorV-0.2.5-legacy-live-test.zip` | Legacy `1.0.3889.0` | D3D11, authenticated CPU-frame bridge |
| `ReactorV-0.2.5-enhanced-live-test.zip` | Enhanced `1.0.1158.13` | D3D12 / D3D11On12, shared GPU frames |
...
0.2.5 hardens late graphics-adapter discovery, native startup-surface readiness,
...
4. For an existing modded installation, use the separate
   `ReactorV-0.2.5-installer.zip` from Releases; its installer preserves the
...
Preloader session logs default to `%LOCALAPPDATA%\ReactorV`. Native hook
and lifecycle diagnostics are written beneath the game's `scripts/ReactorV`
directory, including `ReactorV.RenderHook.log` and `ReactorV.NativeLifecycle.log`.
When reporting an issue, include the edition, exact game version, display mode,
which menu stage failed, and the relevant session log. Review personal paths
before sharing; do not upload unrelated logs or crash dumps publicly.
...
See [the renderer harness guide](DIRECTX-HARNESS.md) and
[live acceptance guide](LIVE-ACCEPTANCE.md) for deeper validation.
