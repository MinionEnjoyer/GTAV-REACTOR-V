...
namespace RageWebUI.Core
{
    ...
    /// starting.  It is deliberately STA-owned and timer-free: the caller
    /// starts its normal paint budget only after a matching ready transition.
    /// </summary>
    public sealed class DeferredNativeSurfaceIntent
    {
        ...
        // Initializer promotion can precede the first provider connection.
        // That connection is not a replacement presentation, so preserve the
        // intent while binding it to the newly authoritative session.
