...
namespace ReactorV.BootstrapAttachmentValidation
{
    internal static class Program
    {
        ...
        {
            using var pair = new PipePair();
            Console.WriteLine("CASE START: partialframe");
            var partialRead = Task.Run(() => CaptureException(() =>
                BootstrapHostWire.Read(pair.Client, 125)));
            ...
            // Closing the client on deadline may fault the peer's final flush;
            // either completion or that expected broken-pipe outcome is valid.
            try { partialWriter.GetAwaiter().GetResult(); }
            catch (IOException) { }
        ...
        {
            using var pair = new PipePair();
            Console.WriteLine("CASE START: disposedpeer");
            var waiting = Task.Run(() => CaptureException(() =>
                BootstrapHostWire.Read(pair.Client, 5000)));
            Thread.Sleep(75);
            pair.Client.Dispose();
            var error = waiting.GetAwaiter().GetResult();
            if (error != null && !(error is IOException) &&
                !(error is ObjectDisposedException) &&
                !(error is OperationCanceledException) && !(error is TimeoutException))
            {
                throw new InvalidOperationException(
                    "Disposing a peer did not unblock the production bounded read.", error);
            }
        ...
        private sealed class PipePair : IDisposable
        {
            public PipePair()
            {
                var name = "ReactorV.BootstrapAttachmentValidation." +
                    Guid.NewGuid().ToString("N");
                Server = new NamedPipeServerStream(name, PipeDirection.InOut, 1,
